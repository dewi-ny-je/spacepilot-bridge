...
**SpacePilot Pro** (or original **SpacePilot**) and a computer, and makes the
computer see a **SpaceMouse Pro Wireless**.
...
| Unknown devices | Any other HID *multi-axis controller* is accepted with the generic V3DK button map (SpaceExplorer, SpaceMouse Pro, ...).  Keyboards and mice plugged into the host port are ignored. |
| Not done | The SpacePilot (Pro) LCD is not driven (it lives on a separate vendor-specific interface and is simply left alone).  No battery report, the device claims to be the cabled variant. |
